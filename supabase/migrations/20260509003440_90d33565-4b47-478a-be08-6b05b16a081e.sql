...
CREATE TABLE IF NOT EXISTS public.lab_tests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  name text NOT NULL,
  -- Short clinical code (e.g. CBC, CMP, HBA1C). LOINC mapping is TODO; populate
  -- a separate loinc_code column later if/when LabCorp interface requires it.
...
-- update_updated_at_column() already exists in the schema (used by other tables).
-- Reuse it here. If it does not exist in some environment, create it.
...
-- ============================================================================
-- 3. RLS POLICIES
-- Anyone authenticated may SELECT (catalog is browsable).
-- Only admin and staff may INSERT/UPDATE/DELETE.
-- ============================================================================
...
-- ============================================================================
-- 5. SEED — lab_tests (23 unique rows; deduplicated across panels)
-- ============================================================================
...
-- ============================================================================
-- 6. SEED — panel_tests join rows
-- Composition follows the "All Foundation tests + …" spec literally, so
-- Foundation's 8 tests are linked into each non-Sexual panel rather than
-- referenced as a parent panel (the schema has no panel→panel relation).
-- Total rows: 8 + 18 + 16 + 13 + 7 = 62.
-- ============================================================================
...
FROM (
  VALUES
    ...
    -- Hormone — Female (8 Foundation + 10 hormone = 18)
    ...
    -- Hormone — Male (8 Foundation + 8 hormone = 16)
    ('hormone-male', 'CBC',    10),
    ('hormone-male', 'CMP',    20),
    ...
    -- Weight Optimization (8 Foundation + 5 metabolic = 13)
    ...
    -- Sexual Wellness (7; CMP-lite delivered as full CMP per panel description)
...
-- ============================================================================
-- Counts (expected: 5 panels, 23 tests, 62 panel_tests join rows):
--   SELECT
--     (SELECT count(*) FROM public.lab_panels)  AS panels,
--     (SELECT count(*) FROM public.lab_tests)   AS tests,
--     (SELECT count(*) FROM public.panel_tests) AS join_rows;
--
-- Sample panel with its test list (data shape check):
--   SELECT
--     p.slug,
--     p.name,
--     p.non_member_price_cents,
...
--         'name', t.name,
--         'category', t.category,
--         'order', pt.display_order
--       ) ORDER BY pt.display_order
--     ) AS tests
--   FROM public.lab_panels p
--   JOIN public.panel_tests pt ON pt.panel_id = p.id
--   JOIN public.lab_tests t ON t.id = pt.test_id
--   WHERE p.slug = 'hormone-female'
--   GROUP BY p.id;
--
-- All panel compositions at a glance:
--   SELECT p.slug, count(pt.*) AS test_count
--   FROM public.lab_panels p
--   LEFT JOIN public.panel_tests pt ON pt.panel_id = p.id
--   GROUP BY p.slug
