...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { patient_name, patient_phone, service_type, payment_url }: ConsultationInviteSMSRequest = await req.json();

    if (!patient_phone || !payment_url) {
      throw new Error("Missing required fields: patient_phone and payment_url");
    }

    logStep("Preparing SMS", { patient_phone, service_type });
    ...
    const message = `Hi ${patient_name || "there"}! Thanks for your interest in ${serviceLabel} at Elevated Health. Book your $99 Strategy Session here: ${payment_url} - This $99 becomes a credit toward treatment. Questions? Call (706) 760-3470`;
