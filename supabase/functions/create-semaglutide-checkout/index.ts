...
serve(async (req) => {
  ...
  try {
    ...
    // Semaglutide Membership - $399/month
    const SEMAGLUTIDE_PRICE_ID = "price_1SlZnwEOtKRY99puaBhrh2iB";
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price: SEMAGLUTIDE_PRICE_ID,
          quantity: 1,
        },
      ],
      mode: "subscription",
      success_url: `${req.headers.get("origin")}/payment-success?type=semaglutide`,
    ...
    if (supabaseUrl && supabaseKey && email) {
      ...
      await supabase.from("consultation_bookings").insert({
        ...
        notes: "Semaglutide Membership subscription - $399/month",
      });
