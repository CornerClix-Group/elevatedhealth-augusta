 ...
 *
 * Receives the public intake form submission and writes patient PHI +
 * symptom_log + safety screening data via the service role.
 *
 * AUTH POSTURE (security audit R-5, 2026-05-08):
 *   - verify_jwt = true in supabase/config.toml — caller must be at least
 *     a logged-in anonymous Supabase user
 *   - The actual auth credential for this function is the
 *     `intake_token` in the request body. The token is generated by
 *     send-welcome-email, persisted on patients.intake_token, has a 7-day
 *     TTL, and is delivered out-of-band via the welcome email. It is a
 *     uuid (effectively unguessable).
 *   - The function looks up the patient WHERE intake_token = body.token
 *     AND intake_token_expires_at > NOW(), and only writes to that
 *     patient row. Any caller without a valid live token gets rejected
 *     before any write occurs.
 *
 * This is the sanctioned "intentionally public via token" pattern. The
 * symptom_log RLS policy was tightened in 20260510040000 to drop the
 * open INSERT — service-role writes from this function continue to work
