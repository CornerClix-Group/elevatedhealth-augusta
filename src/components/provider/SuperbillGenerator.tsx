    ...
    try {
      ...
      if (settingsData) {
        ...
        const settings: ClinicSettings = {
          clinic_legal_name: settingsData.find(s => s.key === "clinic_legal_name")?.value || "Réveil",
    ...
    try {
      ...
      const { error } = await supabase.functions.invoke("send-superbill-email", {
        body: {
          ...
          clinicSettings: {
            legalName: clinicSettings?.clinic_legal_name || "Réveil",
  ...
  const generatePrintableHTML = (selectedCpts: CPTCode[], selectedDiagnoses: ICD10Code[]) => `
    ...
    <body>
      ...
      <div class="watermark">
        ...
        Generated by Réveil | HIPAA Compliant
      </div>
    </body>
    </html>
  `;
